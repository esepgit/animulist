...
function AnimeList({ animes=fake, showAnime }) {
  console.log(animes)
  return (
    <section className="pt-14 mx-4 grid grid-cols-[repeat(auto-fit,_minmax(180px,_1fr))] gap-4 gap-y-10">
      {animes.map(anime => (
        <article key={anime.mal_id} onClick={() => showAnime(anime)} className="relative overflow-hidden cursor-pointer border-2 hover:border.slate-200 border-transparent shadow-slate-400/10 shadow-lg capitalize text-center bg-white rounded-[30px] font-semibold pb-4 group">
